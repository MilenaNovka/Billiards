...
func _process(_delta):
	...
	for b in get_tree().get_nodes_in_group("balls"):
		...
		and b.linear_velocity.length() < MOVE_THRESHOLD):
			...
			moving = true
	if not moving:
		# Verifica se a bola branca foi encacapada e a reseta (teclado padrão americano dá nisso)
		if cue_ball_potted:
			reset_cue_ball()
			cue_ball_potted = false
		if not taking_shot:
...
func potted_ball(body):
	if body == cue_ball:
		...
		# Cria um novo sprite para mostrar a bola encacapada
		var b = Sprite2D.new()
		add_child(b)
